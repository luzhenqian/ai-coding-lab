    ...
    [{ role: "user", content: question }],
    {
      ...
      delegation: {
          ...
          if (prompt) {
            const preview =
              typeof prompt === "string"
                ? prompt.slice(0, 150)
                : JSON.stringify(prompt).slice(0, 150);
            emit({
              type: "stream:chunk",
              nodeId: primitiveId,
              text: `[${primitiveId}] ${preview}...\n`,
            });
          }
        },
        ...
        onDelegationComplete: ({ primitiveId, result: delegationResult }) => {
          if (delegationResult) {
            const text =
              typeof delegationResult === "string"
                ? delegationResult
                : JSON.stringify(delegationResult);
            emit({
              type: "stream:chunk",
              nodeId: primitiveId,
              text:
...
export const supervisorDemo: Demo = {
  meta: {
    ...
    pattern: "Supervisor",
    description:
      "一个 Supervisor LLM 动态调度搜索、分析、写作三个 Agent。执行路径由 LLM 在运行时决定。",
    inputs: [
      {
        id: "question",
        label: "调研问题",
